...
public class NSClientService extends Service {
    ...
    public void onStatusEvent(EventPreferenceChange ev) {
        if (ev.isChanged(R.string.key_nsclientinternal_url) ||
                ...
                ev.isChanged(R.string.key_ns_rest_enable) ||
                ev.isChanged(R.string.key_ns_rest_upload_bg) ||
                ev.isChanged(R.string.key_ns_rest_sleep_seconds)
                ) {
            destroy();
            initialize();
        }
    ...
    public void onStatusEvent(EventConfigBuilderChange ev) {
        if (nsConfig.enabled != MainApp.getSpecificPlugin(NSClientPlugin.class).isEnabled(PluginType.GENERAL)) {
            destroy();
            initialize();
        }
    }
    ...
    public NSConfiguration readPreferences() {
        ...
        conf.enabled = MainApp.getSpecificPlugin(NSClientPlugin.class).isEnabled(PluginType.GENERAL);
        ...
        conf.restEnabled = SP.getBoolean(R.string.key_ns_rest_enable, false);
        conf.restUploadBg = SP.getBoolean(R.string.key_ns_rest_upload_bg, false);
        conf.restSecondsSleep = SP.getInt(R.string.key_ns_rest_sleep_seconds, 60);
        return conf;
    }
    ...
    */
    ...
    public void resend(final String reason, Long afterMillis) {
        getTransportService().resend(reason, afterMillis);
    }
